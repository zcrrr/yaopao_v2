...
@interface CNMatchMainViewController ()

@end

@implementation CNMatchMainViewController
@synthesize nextDis;
@synthesize isIn;
@synthesize big_div;
@synthesize tiv;
...
- (void)initMatch{
    ...
    kApp.match_historydis = 0;
    kApp.match_historySecond = 0;
    kApp.match_startdis = 0;
    kApp.match_currentLapDis = 0;
    kApp.match_countPass = 0;
    kApp.match_time_last_in_track = [CNUtil getNowTimeDelta];
//    kApp.match_pointsString = [NSMutableString stringWithString:@""];
    kApp.match_targetkm = kApp.match_totalDisTeam/kkmInterval+1;//只有match_totalDisTeam没有在这赋值，交接棒会对它赋值
    kApp.match_score = 0;
    kApp.match_km_target_personal = 1;
    kApp.match_km_start_time = 0;
    NSLog(@"目标是%i公里",kApp.match_targetkm);
}
...
- (void)startTimer{
    ...
    [kApp.match_pointList addObject:gpsPoint];
    kApp.match_time_last_in_track = gpsPoint.time;
    //初始化比赛一些参数,正常进入第一个点一定会在赛道上
    if(kApp.isbaton == 1 && kApp.match_totalDisTeam < 1){//是第一棒
    }else{
        //通过gpsPoint初始化
...
//    [kApp.match_pointsString appendString:[NSString stringWithFormat:@"%f,%f",gpsPoint.lon,gpsPoint.lat]];
    [CNAppDelegate match_save2plist];
    [self pushTestArray:[NSString stringWithFormat:@"第一个点:%0.6f,%0.6f",gpsPoint.lon,gpsPoint.lat]];
    [self pushTestArray:[NSString stringWithFormat:@"%0.2f,%0.2f,%i",kApp.match_startdis,kApp.match_currentLapDis,kApp.match_countPass]];
    ...
    [self performSelector:@selector(startReportGPS) withObject:nil afterDelay:delay];
...
- (void)pushOnePoint{
    ...
    if(isInTheTracks){//在赛道内
        ...
        if(timeFromLastInTrack < kBoundary1*60){//小于十分钟
            ...
            self.label_nextArea.text = [NSString stringWithFormat:@"距离下一交接区还有:%0.2f公里",(self.nextDis+5)/1000.0];
...
- (void)pushTestArray:(NSString*)aString{
    return;
    [kApp.array4Test addObject:[NSString stringWithFormat:@"%i:%@",kApp.match_historySecond,aString]];
...
- (void)matchReport{
    ...
    if(kApp.match_totalDisTeam < 1){//是第一棒
        if(self.isStart == NO){
            self.isStart = YES;
            [onepoint setObject:@"0" forKey:@"mstate"];
        }else{
